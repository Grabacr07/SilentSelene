...
namespace MetroRadiance.UI
{
    ...
    public struct Accent : IEquatable<Accent>
    {
        ...
        public static Accent Blue { get; }
            = new Accent(SpecifiedColor.Blue);

        public static Accent Orange { get; }
            = new Accent(SpecifiedColor.Orange);

        public static Accent Purple { get; }
            = new Accent(SpecifiedColor.Purple);

        /// <summary>
        /// Windows のテーマ設定に追従するアクセントを取得します。
        /// </summary>
        public static Accent Windows { get; }
            = new Accent();
